...
/* A struct containing the thread about to sleep, a semaphore used to make
   the thread sleep and the time in ticks to sleep. */
...
timer_sleep (int64_t ticks)
{
  if (ticks > 0) {
    ...
    to_sleep.sleeper = current;
    to_sleep.ticks = ticks;
    sema_init(&(to_sleep.sema),0);

    enum intr_level old_level;
    old_level = intr_disable ();
    list_insert_ordered(&blocked_threads,
                         &to_sleep.elem,
                          &ticks_less_func,
                           NULL);
    intr_set_level (old_level);
    ...
    if (timer_elapsed(start) < ticks) {
      sema_down(&(to_sleep.sema));
    }
  }
}

/* Uses numeric less than on wake_up_ticks to compare two
   elements of the sleeping thread list */
bool
                   ...
                   void *aux UNUSED)
{
  ...
  return a_sleeping->ticks < b_sleeping->ticks;
}
